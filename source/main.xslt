...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:math="http://exslt.org/math"
    ...
    <xsl:template name="draw_scale" match="/Chart">
         <svg>
            ...
            <xsl:variable name="nm_scale_line_single_unit_length_km"><xsl:value-of select="(-1 * (count($maplines/MapLines/ScaleLines/NauticalMilesScaleSingleUnit/Tick) - 1) * $geo_nm_in_meters) div 1000"/></xsl:variable>
            ...
            <xsl:variable name="nm_scale_line_single_unit_start_x"><xsl:value-of select="$km_scale_line_start_x + ((($nm_scale_line_single_unit_length_km * 1000) div $map_zoom) div math:cos($map_base_airport_rwy_latitude * $math_deg_to_radians) ) "/></xsl:variable>
            <xsl:variable name="nm_scale_line_single_unit_end_x"><xsl:value-of select="$km_scale_line_start_x"/></xsl:variable>
            ...
            <xsl:for-each select="$maplines/MapLines/ScaleLines/NauticalMilesScale/Tick">
             ...
             <!-- scale line (single unit) in NM, top thick line-->
            <line>
                <xsl:attribute name="x1"><xsl:value-of select="$nm_scale_line_single_unit_start_x"/></xsl:attribute>
                <xsl:attribute name="y1"><xsl:value-of select="$km_scale_line_start_y + $nm_scale_thick_line_y_separation"/></xsl:attribute>
                <xsl:attribute name="x2"><xsl:value-of select="$nm_scale_line_single_unit_end_x"/></xsl:attribute>
                <xsl:attribute name="y2"><xsl:value-of select="$km_scale_line_end_y + $nm_scale_thick_line_y_separation"/></xsl:attribute>
                <xsl:attribute name="stroke"><xsl:value-of select="$scale_line_color"/></xsl:attribute>
                <xsl:attribute name="stroke-width">4</xsl:attribute>
           </line>

             <!-- scale line (single unit) in NM, bottom line -->
             <xsl:comment>
                 nm_scale_line_single_unit_start_x: <xsl:value-of select="$nm_scale_line_single_unit_start_x"/>
                 nm_scale_line_single_unit_end_x: <xsl:value-of select="$nm_scale_line_single_unit_end_x"/>
                 nm_scale_line_single_unit_length_km : <xsl:value-of select="$nm_scale_line_single_unit_length_km"/>
             </xsl:comment>
            <line>
                <xsl:attribute name="x1"><xsl:value-of select="$nm_scale_line_single_unit_start_x"/></xsl:attribute>
                <xsl:attribute name="y1"><xsl:value-of select="$km_scale_line_start_y + $nm_scale_normal_line_y_separation"/></xsl:attribute>
             ...
             <!-- scale line (single line) in NM, tick lines and text-->
            <xsl:for-each select="$maplines/MapLines/ScaleLines/NauticalMilesScaleSingleUnit/Tick">
                <xsl:variable name="scale_offset"><xsl:value-of select="current()/Offset"/></xsl:variable>
                <line>
                    <xsl:attribute name="x1"><xsl:value-of select="$km_scale_line_start_x - $nm_scale_one_nm_length * $scale_offset"/></xsl:attribute>
                    <xsl:attribute name="y1"><xsl:value-of select="$km_scale_line_start_y + $nm_scale_thick_line_y_separation - $scale_thin_line_thickness -1"/></xsl:attribute>
